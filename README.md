...
_NOTE: In order to run the code you need to set the `GOEXPERIMENT` environment variable to `rangefunc` (e.g. `export GOEXPERIMENT=rangefunc`)._

Go lacks a standard iterator protocol, and this is an attempt to define one.
...
Our example container is a `stack` that is implemented using a linked list.
...
On lines 15-17 we define a `Stack` struct that has a `head` node.
...
If we have more than one iteration going at the same time, the bookkeeping of where each iteration is currently at becomes complex.
Instead, we are going to define an iterator that is responsible for a single iteration.
...
When `it.Next()` returns `false` for the second value it means there are no more items.
...
Now, what if instead of printing we want to save the values to a file, or maybe send them back in an HTTP request handler?
We're not going to write a different function to each scenario.
The solution is to pass to the function that does the iteration another function that will handle the values.
Let's call this value-handling function `yield`.
...
You can see this `Do` pattern in several places in the standard library.
For example: [`fs.WalkDir`](https://pkg.go.dev/io/fs#WalkDir) and [`ring.Do`](https://pkg.go.dev/container/ring#Ring.Do).

What happens if we want to stop the iteration after the first 5 values?
Currently, we can't. 
But we can have the `yield` function return a boolean value indicating that we should stop the iteration.
...
```go
01 type Seq[V any] func(yield func(V) bool)
02 type Seq2[K, V any] func(yield func(K, V) bool)
```

Listing 7 shows the iter.Seq & iter.Seq2 types.

Let's start with `iter.Seq`, it's a function that accepts a `yield` as a parameter.
The `yield` function that accepts a value and returns a `bool`.
...
On line 67 we define `Iter`, it returns a function that matches the `iter.Seq` type.
...
On line 70 we pass the current value to the `yield` function, and if `yield` returns `false` we stop the iteration on line 71.
...
On line 79 we define `Iter2` that returns a function that accepts a function that gets two parameters: an `int` for the index and the value.
...
Once we have generic iteration, we'd want to define a function that work with them.
...
To check for an empty sequence, use `len`.
...
```go
01 func Pull[V any](seq Seq[V]) (next func() (V, bool), stop func())
02 func Pull2[K, V any](seq Seq2[K, V]) (next func() (K, V, bool), stop func())
```

Listing 12 shows the `Pull` and `Pull2` functions.
`Pull` gets a `Seq` and returns two functions: `next` to pull the next value from `seq` and `stop` to signal stop of iteration.
When you call `stop`, the `yield` passed to `seq` will return `false` - signaling it should stop the iteration.
`Pull2` does the same, only with `Seq2` that returns two values.
...
On line 92 we use `iter.Pull` to get `pull` that returns the next value and `stop` that signals to stop the iteration.
...
On line 101 we check if the current value is bigger than `m` (the current maximum) and if so we update `m` on line 102.
...
Listing 14 shows how to use `Max`.
On line 148 we call `Max` on `s.Iter`.
On line 149 we check for an error and if so we print the error on line 150.
Otherwise, on line 152 we print the maximal value.

### Conclusion

The range-over function experiment tries to give Go a general way to provide custom iterations.
...
For me, coming from Python, there are many examples such as linear spaces, generic filters, generic mapper and more.
